...
import ru.yandex.practicum.filmorate.storage.db.rowmappers.UserRowMapper;
...
public class UserDbStorage extends BaseDbStorage<User> implements UserStorage {
    ...
    public UserDbStorage(JdbcTemplate jdbcTemplate, UserRowMapper userRowMapper) {
        ...
        log.debug("Будет использована реализация хранения пользователей в БД");
    }
    ...
    public void addFriend(Long userId, Long friendId) {
        ...
        if (!exists(GET_FRIENDSHIP_ID_QUERY, userId, friendId)) {
            ...
            // Создаём её
            long id = insert(INSERT_FRIENDSHIP_QUERY, userId, friendId);
            if (id == 0) {
                throw new RuntimeException(
                        "Не удалось добавить дружбу между пользователем с id " + userId + "  и пользователем с id "
                                + friendId);
            }
            log.debug("Добавлена новая дружба с id {} между пользователем с id {} и пользователем с id {}", id, userId,
                    friendId);
        } else {
