...
import ru.yandex.practicum.filmorate.storage.db.rowmappers.FilmRowMapper;
...
public class FilmDbStorage extends BaseDbStorage<Film> implements FilmStorage {
    ...
    public FilmDbStorage(JdbcTemplate jdbcTemplate, FilmRowMapper filmRowMapper, GenreStorage genreStorage) {
        ...
        log.debug("Будет использована реализация хранения фильмов в БД");
    ...
    public void addLike(Long filmId, Long userId) {
        ...
        if (!exists(GET_LIKE_ID_QUERY, filmId, userId)) {
            ...
            long id = insert(INSERT_LIKE_QUERY, filmId, userId);
            if (id == 0) {
                throw new RuntimeException(
                        "Не удалось сохранить лайк от пользователя с id " + userId + " фильму с id " + filmId);
            }
            log.debug("Фильму с id {} добавлен лайк от пользователя с id {} с идентификатором связи {}", filmId, userId,
                    id);
        } else {
    ...
    public void addGenre(Long filmId, Long genreId) {
        ...
        if (!exists(GET_GENRE_AND_FILM_LINK_QUERY, filmId, genreId)) {
            ...
            long id = insert(INSERT_GENRE_TO_FILM_QUERY, filmId, genreId);
            if (id == 0) {
                throw new RuntimeException("Не удалось добавить жанр с id " + genreId + " фильму с id " + filmId);
            }
            log.debug("Фильму с id {} добавлен жанр с id {} с идентификатором связи {}", filmId, genreId, id);
        } else {
