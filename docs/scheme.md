...
* Для унификации при проектировании выбраны следующие правила именования сущностей:
    ...
    * `Справедливо для PostgreSQL` Для последовательностей имя должно состоять из префикса `seq_` и названия таблицы.
      Пример: `seq_users`
    * Для первичных и составных ключей имя должно состоять из названия таблицы и постфикса `_pk`. Пример: `users_pk` для
      таблицы
      `users`.
...
| mark    | real       | Нет            | Оценка пользователя        |
...
* таблица содержит проверку `users_films_mark_chk` на значение поля `mark`
